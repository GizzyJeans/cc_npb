...
⚠️ 但校準用的是今天的 —— 舊比賽會換類別 (2026-09-24 發現)
---------------------------------------------------------
slate 在 import 時讀的是 **當下的** `config.calibration_2026`，所以這裡重算的
EV 用的是今天的係數（含那些比賽本身的結果，屬樣本內），不是定價當時的 EV。
上面「用該 slate 自己的門檻」的原則，在校準這一層沒有做到。

影響不小。9/21 → 9/24 校準多了 15 場（798 → 813），同樣的 LEDGER:

    9/21 的校準   A 78   B 28  ROI -13.7%   C 57
    9/24 的校準   A 83   B 27  ROI  -3.4%   C 53

五場舊比賽從 C 換到 A、兩場 B 類的輸局掉到 C、一場 C 類的贏局進了 B ——
**B 類 ROI 跳了 10 個百分點，全是舊比賽換類別**; 9/23 兩場新 B 類只貢獻 -70。
讀這張表的逐日變化時，要先分清是新比賽造成的、還是舊比賽換了類別。

對照: scorecard 的「已下注 vs 未下注」偏誤差距對這件事不敏感 —— 用定價當時
凍結在 .md 裡的模型值算是 1.93 個標準誤，用今天的校準重算是 1.85（個別場次
漂移最多 ±0.5 分，平均 -0.02）。那邊是平均值; 這裡是 **門檻分類**，
EV 在 4% 附近的場次一點點漂移就會翻面。

根治的做法是分類時用定價當時的校準（或把定價當時的 EV 凍結進 LEDGER），
尚未實作。
"""
...
def main() -> None:
    ...
    if n_se >= 2:
        ...
        print(f"- **只有 {n_se:.1f} 個標準誤，還測不出來。** "
              f"{abs(ma - mb) * 100:.0f} 個百分點的 ROI 差距"
    ...
    print("- ⚠️ 分類用的 EV 是以 **今天的校準** 重算（樣本內），不是定價當時的 EV。"
          "EV 在 4% 附近的舊場次會隨每日校準換類別 —— 看逐日變化時先分清"
          "是新比賽、還是舊比賽換了類別（見本檔 docstring 2026-09-24 一節）。")
