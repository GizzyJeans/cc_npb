...
DATE = "2026-08-19"
DATA_AS_OF = "2026-08-19 13:10 JST (UTC 04:10)"
...
GAMES = [
    BoardGame(
        ...
        away_starter="上沢直之 (右)", home_starter="細野晴希 (左)",
    ...
    BoardGame(
        ...
        away_starter="傑瑞 (右)", home_starter="渡邊勇太朗 (右)",
        venue="ベルーナドーム (巨蛋)",
    ...
    BoardGame(
        ...
        away_starter="隆恩 (左)", home_starter="岸孝之 (右)",
...
STARTERS = {
    "日本ハム": ("細野晴希", 0.9151, 6.30, "11 場 69.1 局 防禦率 2.86、79 三振", False),
    "ソフトバンク": ("上沢直之", 0.9225, 6.02, "15 場 90.1 局 防禦率 2.49", False),
    "DeNA": ("東克樹", 0.8724, 6.50, "18 場 117 局 防禦率 2.62、每場 6.5 局", False),
    "巨人": ("井上温大", 0.8002, 6.24, "17 場 106 局 防禦率 2.21、103 三振", False),
    "阪神": ("伊藤将司", 1.0022, 4.94, "6 場 29.2 局 防禦率 3.03（樣本僅 29.2 局）", False),
    "ヤクルト": ("山野太一", 0.7925, 6.30, "18 場 113.1 局 防禦率 2.30", False),
    # 開局投手: 季內單場平均 1.59 局，26 場只投 41.1 局。
    "広島": ("鈴木健矢", 0.8434, 1.59, "26 場 41.1 局 防禦率 2.18（每場僅 1.6 局，後援用法）", True),
    "中日": ("櫻井頼之介", 1.2919, 4.77, "10 場 47.2 局 防禦率 5.66", False),
    "楽天": ("岸孝之", 0.8965, 6.14, "7 場 43 局 防禦率 2.72", False),
    # 開局投手: 季內單場平均 2.51 局。
    "ロッテ": ("ロング", 1.1529, 2.51, "23 場 57.2 局 防禦率 4.53（每場僅 2.5 局，後援用法）", True),
    "西武": ("渡邉勇太朗", 0.9463, 6.61, "17 場 112.1 局 防禦率 3.28", False),
    "オリックス": ("ジェリー", 0.9634, 5.24, "17 場 89 局 防禦率 2.73", False),
}

BULLPEN_NOTE = {
    "日本ハム": "8/16 用 5 人、8/18 用 5 人（伊藤 65 球、上原 42 球、ラオ 36 球）；無連兩日登板",
    "ソフトバンク": "8/16 用 5 人、8/18 用 4 人（モイネロ 98 球）；無連兩日登板",
    "DeNA": "8/16 用 4 人、8/18 只用 3 人（尾形 87 球）；ルイーズ連兩場登板",
    "巨人": "8/16 只用 2 人、8/18 用 4 人（山﨑 97 球）；無連兩日登板 —— 牛棚充足",
    "阪神": "8/16 用 5 人、8/18 用 4 人（村上 97 球）；及川連兩場登板",
    "ヤクルト": "8/16 用 5 人、8/18 用 3 人（吉村 106 球）；星、清水連兩場登板",
    "広島": "8/16 用 5 人、8/18 用 4 人（床田僅 64 球即退場）；菊地、黒原連兩場登板",
    "中日": "8/16 用 5 人、8/18 用 **6 人**（大野 115 球）；橋本、草加、伊藤三人連兩場登板 —— 負荷偏重",
    "楽天": "8/16 用 3 人、8/18 用 4 人（荘司 77 球）；無連兩日登板",
    "ロッテ": "8/16 用 4 人、8/18 用 4 人（吉川首度先發投 90 球）；無連兩日登板",
    "西武": "8/16 用 4 人、8/18 用 4 人（平良 101 球）；無連兩日登板",
    "オリックス": "8/16 用 **6 人**、8/18 又 **6 人**（九里 88 球）；片山連兩場登板 —— 全聯盟負荷最重",
}
