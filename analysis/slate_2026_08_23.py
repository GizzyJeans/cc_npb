...
DATE = "2026-08-23"
DATA_AS_OF = "2026-08-23 16:25 JST (UTC 07:25)"
...
GAMES = [
    BoardGame(
        ...
        away_starter="有原航平 (右)", home_starter="傑克森 (右)",
...
WEATHER = {
    "ZOZOマリン": "露天。**8/22 同一場地已因雨中止一次**，今日為順延重賽；未取得逐時預報",
    "横浜": "露天。**8/22 同一場地已因雨中止一次**，今日為順延重賽；未取得逐時預報",
}

# 今日先發 (npb.jp idp1_<team>.html, 2026-08-15 擷取)。
# is_opener: 季內單場平均局數 < 3 局 —— 實際用法是後援，今天等於開局投手。
STARTERS = {
    "日本ハム": ("有原航平", 1.1724, 6.39, "11 場 70.1 局 防禦率 4.09（失分率 4.86 更差）", False),
    "ロッテ": ("ジャクソン", 0.9263, 6.20, "20 場 124 局 防禦率 3.12、每場 6.2 局、46 四球", False),
    "阪神": ("髙橋遥人", 0.7708, 7.14, "17 場 121.1 局 防禦率 1.93、每場 7.1 局 —— 全聯盟頂級", False),
    "DeNA": ("石田裕太郎", 0.8995, 6.07, "15 場 91 局 防禦率 2.97", False),
}

BULLPEN_NOTE = {
    "日本ハム": "8/21 用 5 人、8/22 因雨中止未登板 —— 牛棚全休",
    "ロッテ": "8/21 用 4 人、8/22 因雨中止未登板 —— 牛棚全休",
    "阪神": "8/21 用 5 人、8/22 因雨中止未登板 —— 牛棚全休",
    "DeNA": "8/21 用 4 人、8/22 因雨中止未登板 —— 牛棚全休",
}
