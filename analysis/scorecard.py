...
def main() -> None:
    ...
    # ---------- 露天 vs 巨蛋 ----------
    # 露天 EV 門檻曾經是 +7% (2026-08-16 設立、2026-09-02 撤除)，用來補償
    # 「拿不到當日天氣」。它只有在露天的誤差真的比較大時才站得住，所以要
    # 持續追蹤，不能只在露天場出現大失誤的那天才臨時查一次 ——
    # 撤除的依據正是這一節連續多日都看不到差距。
    print("\n## 露天 vs 巨蛋（露天 EV 門檻的依據）\n")
    print("> 2026-09-02: 依此表連續多日的結果，露天的 +7% EV 門檻已撤除、"
          "降回一般的 +4%。見 `bethero.gates.OPEN_AIR_MIN_EV`。"
          "本節繼續追蹤 —— 若差距日後真的長出來且超過 2 個標準誤，"
          "就該把門檻加回去。\n")
