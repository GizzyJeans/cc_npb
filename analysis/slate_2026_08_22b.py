...
DATE = "2026-08-22"
DATA_AS_OF = "2026-08-22 17:40 JST (UTC 08:40)"
...
GAMES = [
    BoardGame(
        ...
        away_starter="達孝太 (右)", home_starter="傑克森 (右)",
    ...
    BoardGame(
        ...
        away_starter="隅田知一郎 (左)", home_starter="早川隆久 (左)",
...
OPEN_AIR = {"ZOZOマリン", "楽天モバイル", "横浜"}   # 今日這三場全露天

DAILY_BUDGET = 1000.0
"""本檔是 8/22 的 **第二批** (18:00 JST 三場)。

使用者指定的單日上限是 3,000，而同日 14:00 那批已用掉 2,000
(養樂多@中日 小分、歐力士@軟銀 大分)，所以這一批只剩 1,000 額度。
上限是「單日」不是「單張看板」，兩批必須合起來算。"""
...
STARTERS = {
    "日本ハム": ("達孝太", 0.9372, 4.08, "22 場 89.2 局 防禦率 3.11（每場 4.1 局，偏短）", False),
    "ロッテ": ("ジャクソン", 0.9241, 6.20, "20 場 124 局 防禦率 3.12、每場 6.2 局", False),
    "西武": ("隅田知一郎", 0.9076, 7.06, "18 場 127 局 防禦率 2.48、每場 7.1 局", False),
    "楽天": ("早川隆久", 0.8575, 6.69, "16 場 107 局 防禦率 2.78、108 三振", False),
    # 開局投手: 3 場 8 局、每場僅 2.7 局，且樣本極小。
    "阪神": ("今朝丸裕喜", 1.1282, 2.67, "3 場 8 局 防禦率 5.63（每場僅 2.7 局，後援用法且樣本僅 8 局）", True),
    "DeNA": ("竹田祐", 1.3256, 4.47, "5 場 22.1 局 防禦率 6.04（失分率 8.46 遠高於防禦率，樣本僅 22.1 局）", False),
}

BULLPEN_NOTE = {
    "日本ハム": "8/20 用 **7 人**（12 局和局）、8/21 用 5 人；本週負荷最重",
    "ロッテ": "8/20 用 4 人、8/21 用 5 人（ジャクソン 前登板 115 球）",
    "西武": "8/20 用 4 人、8/21 用 4 人；無連兩日登板",
    "楽天": "8/20 用 4 人、8/21 用 5 人（前田健 113 球）",
    "阪神": "8/20 用 5 人、8/21 用 4 人（下村 101 球）；今日又派每場僅 2.7 局的開局投手",
    "DeNA": "8/20 用 **6 人**、8/21 用 4 人（平良 前登板退場早）",
}
